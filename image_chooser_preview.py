...
import folder_paths
from custom_nodes.ComfyUI_Sound_NODE import huiben
...
class PreviewAndChoose(PreviewImage):
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                "mode": (
                    ["Always pause", "Repeat last selection", "Only pause if batch", "Progress first pick",
                     "Pass through",
                     "Take First n", "Take Last n"], {}),
                "count": ("INT", {"default": 1, "min": 1, "max": 999, "step": 1}),

            },
            "optional": {"images": ("IMAGE",), "latents": ("LATENT",), "masks": ("MASK",), "path": ("STRING", {"default": "ComfyUI"}),},
    ...
    def func(self, id, **kwargs):
        ...
        global path
        if  kwargs.get('path'):
            path = str(kwargs.pop('path', [1, ])[0])
        else:
            path = self.path
        # any other parameters shouldn't be lists any more...
        for x in kwargs: kwargs[x] = kwargs[x][0]
        # call PreviewImage base
        #ret =  huiben.sa(self, images=images_in,filename_prefix=path,prompt=None,extra_pnginfo=None,)
        ...
        try:
            ...
            selections = MessageHolder.waitForMessage(id, asList=True) if (
                    is_blocking_mode and is_block_condition) else [0]
        except Cancelled:
            raise InterruptProcessingException()
            # return (None, None,)
        return self.batch_up_selections(images_in=images_in, latent_samples_in=latent_samples_in, masks_in=masks_in,
    ...
    def tensor_bundle(self, tensor_in: torch.Tensor, picks):
        if tensor_in is not None and len(picks):
            ...
            my_tuple = tuple([tensor_in[(x) % batch].unsqueeze_(0) for x in picks])
            for i in range(len(my_tuple)):
                self1 = {"output_dir": folder_paths.get_output_directory(), "type": "output", "prefix_append": "\"",
                         "compress_level": 4}
                a = self.path + str(picks[i]);
                huiben.saveImageNew(self=self1, extra_pnginfo=None, filename_prefix=a,
                                    images=my_tuple[i], prompt=None)
            return torch.cat(my_tuple).reshape([-1] + list(tensor_in.shape[1:]))
        else:
            return None
    ...
    def batch_up_selections(self, images_in: torch.Tensor, latent_samples_in: torch.Tensor, masks_in: torch.Tensor,
                            selections, mode):
        ...
        return (self.tensor_bundle(images_in, chosen), self.latent_bundle(latent_samples_in, chosen),
                self.tensor_bundle(masks_in, chosen), ",".join(str(x) for x in chosen),)
